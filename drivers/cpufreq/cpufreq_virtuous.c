 ...
 */
#define DEFAULT_UP_MIN_FREQ 972000
 ...
 */
#define DEFAULT_RAMP_UP_STEP 648000
...
static void cpufreq_virtuous_freq_change_time_work(struct work_struct *work)
{
	...
	this_virtuous = &per_cpu(virtuous_info, 0);
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		this_virtuous->cur_policy->max = new_policy->max;
		this_virtuous->cur_policy->min = new_policy->min;
		this_virtuous->cur_policy->cur = new_policy->max;
...
{	
	...
	up_wq = create_workqueue("kvirtuous_up");
