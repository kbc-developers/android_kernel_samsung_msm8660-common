...
static struct snd_pcm_hardware msm_pcm_hardware_capture = {
	...
	.channels_min =         1,
	.channels_max =         8,
	.buffer_bytes_max =     CAPTURE_MAX_PERIODS * CAPTURE_MAX_PERIOD_SIZE,
	.period_bytes_min =	    CAPTURE_MIN_PERIOD_SIZE,
	.period_bytes_max =     CAPTURE_MAX_PERIOD_SIZE,
	.periods_min =          CAPTURE_MIN_PERIODS,
	.periods_max =          CAPTURE_MAX_PERIODS,
	.fifo_size =            0,
};
...
static struct snd_pcm_hardware msm_pcm_hardware_playback = {
	...
	.channels_min =         1,
	.channels_max =         6,
	.buffer_bytes_max =     PLAYBACK_MAX_PERIODS * PLAYBACK_MAX_PERIOD_SIZE,
	.period_bytes_min =     PLAYBACK_MIN_PERIOD_SIZE,
	.period_bytes_max =     PLAYBACK_MAX_PERIOD_SIZE,
	.periods_min =          PLAYBACK_MIN_PERIODS,
	.periods_max =          PLAYBACK_MAX_PERIODS,
	.fifo_size =            0,
};
		...
		uint32_t token, uint32_t *payload, void *priv)
{
	...
	case ASM_DATA_EVENT_WRITE_DONE: {
		...
		wake_up(&prtd->the_locks.write_wait);
		if (!atomic_read(&prtd->start))
			break;
		if (!prtd->mmap_flag)
			break;
	...
	case ASM_DATA_CMDRSP_EOS:
		...
		prtd->cmd_ack = 1;
		wake_up(&prtd->the_locks.eos_wait);
		break;
	case ASM_DATA_EVENT_READ_DONE: {
		...
		prtd->in_frame_info[token][0] = payload[2];
		prtd->in_frame_info[token][1] = payload[3];
		prtd->pcm_irq_pos += prtd->in_frame_info[token][0];
		pr_debug("pcm_irq_pos=%d\n", prtd->pcm_irq_pos);
		if (atomic_read(&prtd->start))
			snd_pcm_period_elapsed(substream);
		if (atomic_read(&prtd->in_count) <= prtd->periods)
			atomic_inc(&prtd->in_count);
		wake_up(&prtd->the_locks.read_wait);
		if (prtd->mmap_flag)
...
static int msm_pcm_open(struct snd_pcm_substream *substream)
{
	...
	msm_session_open(substream->pcm->device, substream->stream,
		prtd->audio_client);
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		ret = snd_pcm_hw_constraint_minmax(runtime,
			...
			PLAYBACK_MIN_PERIODS * PLAYBACK_MIN_PERIOD_SIZE,
			PLAYBACK_MAX_PERIODS * PLAYBACK_MAX_PERIOD_SIZE);
		if (ret < 0) {
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
		ret = snd_pcm_hw_constraint_minmax(runtime,
			SNDRV_PCM_HW_PARAM_BUFFER_BYTES,
			CAPTURE_MIN_PERIODS * CAPTURE_MIN_PERIOD_SIZE,
			CAPTURE_MAX_PERIODS * CAPTURE_MAX_PERIOD_SIZE);
		if (ret < 0) {
			pr_err("constraint for buffer bytes min max ret = %d\n", ret);
		}
	}

	init_waitqueue_head(&prtd->the_locks.eos_wait);
	init_waitqueue_head(&prtd->the_locks.write_wait);
	init_waitqueue_head(&prtd->the_locks.read_wait);
	...
	snd_pcm_uframes_t hwoff, void __user *buf, snd_pcm_uframes_t frames)
{
	...
	ret = wait_event_timeout(prtd->the_locks.write_wait,
...
static int msm_pcm_playback_close(struct snd_pcm_substream *substream)
{
	...
	dir = IN;
	ret = wait_event_timeout(prtd->the_locks.eos_wait,
				prtd->cmd_ack, 5 * HZ);
	if (ret < 0)
	...
	msm_session_close(substream->pcm->device, SNDRV_PCM_STREAM_PLAYBACK);
	q6asm_audio_client_free(prtd->audio_client);
	kfree(prtd);
						 ...
						 snd_pcm_uframes_t frames)
{
	...
	ret = wait_event_timeout(prtd->the_locks.read_wait,
	...
	if (bufptr) {
		...
		if (xfer > size)
			xfer = size;
		offset = prtd->in_frame_info[idx][1];
...
static int msm_pcm_capture_close(struct snd_pcm_substream *substream)
{
	...
	q6asm_audio_client_buf_free_contiguous(dir,
				prtd->audio_client);
	msm_session_close(substream->pcm->device, SNDRV_PCM_STREAM_CAPTURE);
	q6asm_audio_client_free(prtd->audio_client);
	kfree(prtd);
